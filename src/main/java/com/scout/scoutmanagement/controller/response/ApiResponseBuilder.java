package com.scout.scoutmanagement.controller.response;

import com.scout.scoutmanagement.domain.Actividad;
import com.scout.scoutmanagement.domain.Curso;
...
public class ApiResponseBuilder {
    ...
    public Map<String, Object> evento(String mensaje, Evento evento) {
        ...
        if (evento instanceof Actividad actividad) {
        ...
        if (evento instanceof Curso curso) {
            respuesta.put("nivelCurso", curso.getNivel());
            respuesta.put(
                "suscriptosCursoIds",
                curso.getSuscriptos().stream().map(Persona::getId).toList()
            );
        }
    ...
    private Map<String, Object> eventoCalendario(Evento eventoEntidad) {
        ...
        if (eventoEntidad instanceof Actividad actividad) {
            evento.put("educadorResponsableId", actividad.getEducadorResponsable().getId());
            evento.put("beneficiarioACargoId", actividad.getBeneficiarioACargo().getId());
        }

        if (eventoEntidad instanceof Curso curso) {
            evento.put("nivelCurso", curso.getNivel());
            evento.put("suscriptosCursoIds", curso.getSuscriptos().stream().map(Persona::getId).toList());
        }
